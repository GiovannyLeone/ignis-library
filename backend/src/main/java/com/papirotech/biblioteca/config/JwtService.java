...
public class JwtService {
    ...
    public boolean isTokenValido(String token, UserDetails u) {
        return extrairUsername(token).equals(u.getUsername()) && !extrairClaim(token,Claims::getExpiration).before(new Date());
    }
    private <T> T extrairClaim(String token, Function<Claims,T> fn) {
